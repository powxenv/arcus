// Result-pipeline data layer — SERVER ONLY.
//
// This module imports `db` (which imports `cloudflare:workers`) and must never
// be imported from client code. It is imported only by:
//   - src/server/results.ts        (the createServerFn wrappers the client calls)
//   - src/server/ai-context.ts     (server-side AI context builder)
//   - src/routes/api/results/*     (the REST server routes)
//
// Why a separate file: TanStack Start extracts `createServerFn` exports so the
// client never bundles their bodies. But if a file mixes `createServerFn`
// exports with *plain* functions that touch `cloudflare:workers`, the client
// build pulls the whole module and fails to resolve `cloudflare:workers`
// (cloudflare/workers-sdk#10254). Keeping the DB-touching code here, out of the
// client-imported server-fn file, is the fix.
...
export function validateUpdateAIInput(raw: unknown): {
  shareToken: string;
  aiAnalysis: string | null;
} {
  if (typeof raw !== "object" || raw === null) throw new Error("Invalid payload");
